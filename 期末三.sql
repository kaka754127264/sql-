...
5、使用select语句完成数据的查询需求（简单查询、where条件查询、分组查询、多表连接查询，子查询） (20分)
   ...
   select * from post_send full join QM_User on post_send.user_l = QM_User.user_l
	...
	--分组查询
	select post_send.user_l ,count(send_id) as 人数 ,send_id,send_tel,send_remark,QM_User.user_name from post_send
	...
	--子查询/嵌套查询（查询我自己——个人等级大于14级，小于16级）
		select * from post_send
		where user_l >(select user_l from post_return
		where return_remark = '天秀')
		INTERSECT
		select * from post_send
		where user_l <(select user_l from post_return
		where return_remark = '蒂花之秀')
...
/*
   ...
   select * from post_return full join QM_User on post_return.user_l = QM_User.user_l
	...
	--分组查询
	select post_return.user_l ,count(return_id) as 人数 ,return_id,return_tel,return_remark,QM_User.user_name from post_return
...
/*
6、视图() (10分）
   设计一个你认为系统重要的查询需求，用SQL语句查询并用视图的方式实现
   **/
	...
	where user_name like '宋%'




/***
7、使用T-SQL实现简单的业务流程（编程） (10分)
   自行设计一个业务处理程序，过程、函数或者触发器来实现,自选一个实现即可,
/*程序说明:该程序实现XXXXXX功能*/
程序代码:
*/
...
as
begin
	select post_return.user_l ,count(return_id) as 人数 ,return_id,return_tel,return_remark,QM_User.user_name from 
post_return
	full join QM_User on post_return.user_l = QM_User.user_l
